...
func (c *ClientImpl) GetSSHKeyPath(serviceId, sshKeyId string) string {
	return c.getServicePath(serviceId, fmt.Sprintf("/clickpipesSSHKeys/%s", sshKeyId))
}

func (c *ClientImpl) ListSSHKeys(ctx context.Context, serviceId string) ([]*SSHKey, error) {
	req, err := http.NewRequest(http.MethodGet, c.getServicePath(serviceId, "/clickpipesSSHKeys"), nil)
	if err != nil {
		return nil, err
	}
...
func (c *ClientImpl) CreateSSHKey(ctx context.Context, serviceId string, request CreateSSHKey) (*SSHKey, error) {
	...
	req, err := http.NewRequest(http.MethodPost, c.getServicePath(serviceId, "/clickpipesSSHKeys"), &payload)
	if err != nil {
		return nil, err
	}
...
func (c *ClientImpl) UpdateSSHKey(ctx context.Context, serviceId, sshKeyId string, request UpdateSSHKey) (*SSHKey, error) {
	var payload bytes.Buffer
	if err := json.NewEncoder(&payload).Encode(request); err != nil {
		return nil, fmt.Errorf("failed to encode SSHKey update: %w", err)
	}

	req, err := http.NewRequest(http.MethodPatch, c.GetSSHKeyPath(serviceId, sshKeyId), &payload)
	if err != nil {
		return nil, err
	}
...
func (c *ClientImpl) ValidateSSHKey(ctx context.Context, serviceId, sshKeyId string) (*SSHKey, error) {
	req, err := http.NewRequest(http.MethodPost, c.GetSSHKeyPath(serviceId, sshKeyId)+"/validate", nil)
	if err != nil {
		return nil, err
	}
