...
func Test_getGrantPrivilegeQuery(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			granteeUserName: toStrPtr("john"),
			granteeRoleName: nil,
			query:           "SELECT access_type, database, table, column, user_name, role_name, toBool(grant_option) as grant_option FROM system.grants WHERE access_type = ? AND database = ? AND user_name = ?",
		...
		},
		{
			...
			granteeUserName: nil,
			granteeRoleName: toStrPtr("writer"),
			query:           "SELECT access_type, database, table, column, user_name, role_name, toBool(grant_option) as grant_option FROM system.grants WHERE access_type = ? AND database = ? AND role_name = ?",
		...
		},
		{
			...
			granteeUserName: toStrPtr("john"),
			granteeRoleName: nil,
			query:           "SELECT access_type, database, table, column, user_name, role_name, toBool(grant_option) as grant_option FROM system.grants WHERE access_type = ? AND database = ? AND table = ? AND user_name = ?",
