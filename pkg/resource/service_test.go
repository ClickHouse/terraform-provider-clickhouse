...
func TestServiceResource_ModifyPlan_horizontal(t *testing.T) {
	...
	t.Run("production create omitting autoscaling_mode still requires the memory fields", func(t *testing.T) {
		// A real create leaves the omitted Optional+Computed per-replica fields Unknown (not Null); the guard
		// must still fire, or a memory-less production create POSTs a 0 GiB bound.
...
func TestServiceResource_ModifyPlan_modeSwitch(t *testing.T) {
	...
	t.Run("vertical num_replicas -> vertical equal-band migration nulls the stale num_replicas", func(t *testing.T) {
		// The documented migration: an existing vertical service using num_replicas moves to the equal
		// min==max band replacement, dropping num_replicas and omitting the mode. The stale num_replicas
		// pinned by UseStateForUnknown must be nulled, or Update omits it (plan==state), the API clears it,
		// and the read-back disagrees with the planned value ("inconsistent result after apply").
		...
		// Plan: UseStateForUnknown pins the prior vertical mode + num_replicas; the band takes the new values.
		plan := vertical(func(s *models.ServiceResourceModel) {
			s.MinReplicas = types.Int64Value(3)
			s.MaxReplicas = types.Int64Value(3)
		})
		out := run(t, vertical(nil), bandConfig, plan)
		if out.AutoscalingMode.ValueString() != api.AutoscalingModeVertical {
			t.Errorf("an equal-band migration must stay vertical; got %v", out.AutoscalingMode)
		}
		if !out.NumReplicas.IsNull() {
			t.Errorf("the stale num_replicas must be nulled on the equal-band migration; got %v", out.NumReplicas)
		}
		if out.MinReplicas.ValueInt64() != 3 || out.MaxReplicas.ValueInt64() != 3 {
			t.Errorf("the equal band must be preserved; got min=%v max=%v", out.MinReplicas, out.MaxReplicas)
		}
	})
