...
- `generated_password` (String, Sensitive) The password ClickHouse assigned to the default user at creation, when you don't supply your own (`password` and `password_hash` unset, and `password_wo = ""`). To use this: set `password_wo = ""` and `password_wo_version` (e.g. `1`) — the schema still requires one of `password`/`password_hash`/`password_wo` to be set, so this is how you opt into an auto-generated password instead of choosing your own. Only set right after creation. It goes blank again if you later change the password through this provider, and it isn't updated if the password changes outside Terraform (console, another tool) — so treat it as a one-time value to capture at creation, not a live mirror of the current password. Useful when the caller creating the service shouldn't need permission to reset its password afterward.
