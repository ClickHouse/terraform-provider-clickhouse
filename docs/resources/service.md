...
- `snapshot_configuration` (Attributes) Configuration of service snapshot settings. Snapshots are a beta feature that requires the snapshots feature enabled for your organization on a PPv2 (Scale or Enterprise) tier; eligibility is not validated at plan time, so an ineligible organization will fail at apply after the service is created, leaving it untracked in state (delete it manually). When enabled, gap and time_frame must be supplied together as a supported preset: (gap 30, time_frame 1440) or (gap 60, time_frame 2880). (see [below for nested schema](#nestedatt--snapshot_configuration))
