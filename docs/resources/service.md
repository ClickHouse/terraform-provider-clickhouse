...
- `max_replica_memory_gb` (Number) Maximum memory of a single replica in GiB. For vertical autoscaling this is the high end of the memory range; for horizontal it must equal min_replica_memory_gb (per-replica memory is fixed).
- `max_replicas` (Number) Maximum number of replicas. For horizontal autoscaling this is the high end of the band; for vertical it must equal min_replicas. Conflicts with num_replicas.
- `max_total_memory_gb` (Number, Deprecated) Maximum total memory of all workers during auto-scaling in GiB.
- `min_replica_memory_gb` (Number) Minimum memory of a single replica in GiB. For vertical autoscaling this is the low end of the memory range; for horizontal it must equal max_replica_memory_gb (per-replica memory is fixed).
- `min_replicas` (Number) Minimum number of replicas. For horizontal autoscaling this is the low end of the band; for vertical it must equal max_replicas (the go-forward replacement for num_replicas). Conflicts with num_replicas.
- `min_total_memory_gb` (Number, Deprecated) Minimum total memory of all workers during auto-scaling in GiB.
- `num_replicas` (Number) Legacy fixed replica count for a vertical service. Conflicts with min_replicas/max_replicas. Forbidden when autoscaling_mode is "horizontal".
