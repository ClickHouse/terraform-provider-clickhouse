...
description: |-
  ...
  Pin a version: set version to a fixed number. Terraform keeps that version attached until you change it.Track the latest version: set version = clickhouse_udf.<name>.version. Each new UDF version changes this value, so Terraform attaches it on the next apply.Attach once, then leave alone: omit version. Terraform attaches whatever is ready at create time and stores that number in state. Publishing a newer version afterwards does not move the attachment, since there's no config value telling Terraform to.
...
- `version` (Number) Version to attach. When omitted, the latest ready version is attached.
