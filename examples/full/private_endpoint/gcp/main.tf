...
resource "clickhouse_service_private_endpoints_attachment" "this" {
  private_endpoint_ids = [tostring(google_compute_forwarding_rule.psc_endpoint.psc_connection_id)]
  service_id           = clickhouse_service.this.id
}

# hostname for connecting to instance via Private Service Connect from the VPC
output "private_endpoint_hostname" {
